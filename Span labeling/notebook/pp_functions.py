...
def make_embedding_layers(train, output_dim=20, feature='affix'):
    ...
    for i in range(len(train.vector_size[feature])):
        ...
        layer_dict['masks'].append(layers.Masking(mask_value = 0)(layer_dict['inputs'][-1]))
        layer_dict['embeddings'].append(layers.Embedding(input_dim=train.vector_size[feature][i]+2, output_dim=output_dim)(layer_dict['masks'][-1]))
    return layer_dict
...
def pos_tag(X,max_len,s='numbers'):
    ...
    for i in X:
        ...
        for j in tag:
            l.append(j[1])
            on_hot.append(np.zeros(len(tags)))
            z.append(tags.index(j[1]))
        pos_tag.append(l)
        one_hot_pos.append(on_hot)
        index.append(z)
        
    final = []
    max_l = []
    for i,j in zip(one_hot_pos,index):
        end = []
        for x,z in zip(i,j):
            temp = x
            temp[z] = 1
...
def map_df1(word,tfidf_dat,tfidf_obj):
    return tfidf_dat.T[tfidf_obj.vocabulary_[word]]

def tfidf_repr(data,max_len,tfidf_obj,tfidf_dat):
    final = []
    fin = []
    for i in data:
        ls = []
        for x in i:
            if x in tfidf_obj.vocabulary_:
                ls.append(map_df1(x,tfidf_dat,tfidf_obj).toarray().flatten())
            else:
                ls.append(np.zeros(1744))
        final.append(ls)
    for i in range(len(final)):
        for i in range(max_len):
            if len(final[i]) < max_len:
                final[i].append(np.zeros(1744))
        fin.append(final[i])
    return np.array(fin)
